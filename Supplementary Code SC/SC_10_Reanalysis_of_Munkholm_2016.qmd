---
title: "Reanalysing Munkholm et al. (2016)"
author: ""
format: pdf
editor: visual
root.dir: ""
...
```{r}
#| label: munkholm_data_prep
library(dplyr)
library(psychometric)
library(officer)
library(stringr)
X <- read.csv2("../Supplementary Data SD/SD 10.1 munkholm_2016.csv")
X[, 3:5] <- sapply(X[, 3:5], as.numeric)

X_ref <- docx_summary(read_docx("../Supplementary Data SD/SD 10.2 munkholm_2016_ref.docx"))$text

X_authors <- sapply(str_split(X_ref, "\\."), "[[", 1)
X_authors <- gsub(" ", "", X_authors)
X_authors <- gsub("GoiJD", "GoiJ", X_authors)
X_authors <- gsub("KapczinskiF", "KapczinskyF", X_authors)
X_authors <- gsub("Kauer-Sant'annaM", "Kauer-Sant'AnnaM", X_authors)
X_authors <- gsub("MagalhaesPVS", "MagalhaesPV", X_authors)
X_authors <- gsub("Wollenhaupt-AguiarB", "AguiarB", X_authors)
...
```{r}
#| label: munkholm_bipartite_edges
...
```{r}
#| label: munkholm_coauthor_overlap_network
#| fig-width: 12
#| fig-height: 12
#| warning: false
library(statnet)
...
```{r}
#| label: munkholm_effect_sizes
library(metafor)
X_effect <- X
X_effect$yi <- X_effect$hedgesg
X_effect$se <- (X_effect$uplim - X_effect$lowlim) / 3.92
X_effect$vi <- X_effect$se^2
```

## 3.2. Correlogram

```{r}
#| label: munkholm_correlogram
...
```{r}
#| label: munkholm_similarity_matrix_invgeo
...
```{r}
#| label: munkholm_similarity_matrix_jaccard
...
```{r}
#| label: munkholm_similarity_matrix_jaccard_correlogram
jaccard_vec <- vector()
for(i in 1:length(unique(X$study_id))){
...
```{r}
#| label: munkholm_similarity_matrix_invgeo_jaccard
...
```{r}
#| label: munkholm_null_model
X.rma.null <- rma.mv(yi, vi, random = list(~1|study_id/obs),
                      data = X_effect)
save(X.rma.null, file = "munkholm.rma.null.Rdata")
```

### 3.4.3. Inv.geo model

```{r}
#| label: munkholm_invgeo_model
X.rma.full.invgeo <- rma.mv(yi, V = vi, random = list(~1|study_id/obs,
                                                         ~1|study_id.phyl),
              R = list(study_id.phyl = X_inv.geo), data = X_effect)
save(X.rma.full.invgeo, file = "munkholm.rma.full.invgeo.Rdata")
```

### 3.4.4. Jaccard model

```{r}
#| label: munkholm_jaccard_model
X.rma.full.jaccard <- rma.mv(yi, V = vi, random = list(~1|study_id/obs,
                                                         ~1|study_id.phyl),
              R = list(study_id.phyl = X_jaccard_matrix), data = X_effect)
save(X.rma.full.jaccard, file = "munkholm.rma.full.jaccard.Rdata")
```

### 3.4.5. Inv.geo + Jaccard model

```{r}
#| label: munkholm_invgeo_jaccard_model
X.rma.full.invgeo_jaccard <- rma.mv(yi, V = vi, random = list(~1|study_id/obs,
                                                         ~1|study_id.phyl),
              R = list(study_id.phyl = X_invgeo_jaccard), data = X_effect)
save(X.rma.full.invgeo_jaccard, file = "munkholm.rma.full.invgeo_jaccard.Rdata")
```

### 3.4.6. Summary

```{r}
#| label: munkholm_rma_all
load("munkholm.rma.null.Rdata")
load("munkholm.rma.full.invgeo.Rdata")
load("munkholm.rma.full.jaccard.Rdata")
load("munkholm.rma.full.invgeo_jaccard.Rdata")
summary(X.rma.null)
...
```{r}
#| label: munkholm_model_comparison_plot
...
```{r}
#| label: munkholm_adjusted_effects
############################ invgeo ##############################
############################ invgeo ##############################
...
```{r}
#| label: munkholm_correlogram_adjusted
#| fig.width: 12
#| fig.height: 12
#################### CIs for invgeo ###########################
######## raw ##########
...
################## plot ##########################################
# combined plot:
par(mar = c(4,4,4,1), family = "serif", cex = 2)
...
# raw jaccard
lines(seq(1, 4, len = 7), rev(X_mantel_corlog_jaccard$correlation)[1:7], lwd = 2, col = "blue")
points(seq(1, 4, len = 7), rev(X_mantel_corlog_jaccard$correlation)[1:7], col = "blue", cex = .5)
points(5.05, X_mantel_corlog_jaccard$correlation[1], col = "blue", cex = .5)
arrows(seq(1, 4, len = 7)[7], y0 = corlog_CI_jaccard[2, 1], y1 = corlog_CI_jaccard[2, 2], length = .05, angle = 90, code = 3, col = "blue")
arrows(seq(1, 4, len = 7)[6], y0 = corlog_CI_jaccard[3, 1], y1 = corlog_CI_jaccard[3, 2], length = .05, angle = 90, code = 3, col = "blue")
arrows(seq(1, 4, len = 7)[5], y0 = corlog_CI_jaccard[4, 1], y1 = corlog_CI_jaccard[4, 2], length = .05, angle = 90, code = 3, col = "blue")
arrows(seq(1, 4, len = 7)[4], y0 = corlog_CI_jaccard[5, 1], y1 = corlog_CI_jaccard[5, 2], length = .05, angle = 90, code = 3, col = "blue")
arrows(seq(1, 4, len = 7)[3], y0 = corlog_CI_jaccard[6, 1], y1 = corlog_CI_jaccard[5, 2], length = .05, angle = 90, code = 3, col = "blue")
arrows(seq(1, 4, len = 7)[2], y0 = corlog_CI_jaccard[7, 1], y1 = corlog_CI_jaccard[5, 2], length = .05, angle = 90, code = 3, col = "blue")
arrows(5.05, y0 = corlog_CI_jaccard[1, 1], y1 = corlog_CI_jaccard[1, 2], length = .05, angle = 90, code = 3, col = "blue")
# adjusted jaccard
lines(seq(1, 4, len = 7), rev(ad_mantel_corlog_jaccard$correlation)[1:7], lwd = 2, lty = 2, col = "blue")
points(seq(1, 4, len = 7), rev(ad_mantel_corlog_jaccard$correlation)[1:7], col = "blue", cex = .5)
points(5.05, ad_mantel_corlog_jaccard$correlation[1], col = "blue", cex = .5)
arrows(seq(1, 4, len = 7)[7], y0 = corlog_CI_ad_jaccard[2, 1], y1 = corlog_CI_ad_jaccard[2, 2], length = .05, angle = 90, code = 3, col = "blue")
arrows(seq(1, 4, len = 7)[6], y0 = corlog_CI_ad_jaccard[3, 1], y1 = corlog_CI_ad_jaccard[3, 2], length = .05, angle = 90, code = 3, col = "blue")
arrows(seq(1, 4, len = 7)[5], y0 = corlog_CI_ad_jaccard[4, 1], y1 = corlog_CI_ad_jaccard[4, 2], length = .05, angle = 90, code = 3, col = "blue")
arrows(seq(1, 4, len = 7)[4], y0 = corlog_CI_ad_jaccard[5, 1], y1 = corlog_CI_ad_jaccard[5, 2], length = .05, angle = 90, code = 3, col = "blue")
arrows(seq(1, 4, len = 7)[3], y0 = corlog_CI_ad_jaccard[6, 1], y1 = corlog_CI_ad_jaccard[5, 2], length = .05, angle = 90, code = 3, col = "blue")
arrows(seq(1, 4, len = 7)[2], y0 = corlog_CI_ad_jaccard[7, 1], y1 = corlog_CI_ad_jaccard[5, 2], length = .05, angle = 90, code = 3, col = "blue")
arrows(5.05, y0 = corlog_CI_ad_jaccard[1, 1], y1 = corlog_CI_ad_jaccard[1, 2], length = .05, angle = 90, code = 3, col = "blue")
# rest:
abline(v = 4.48)
abline(v = 4.52)
axis(side = 1, labels = c(0:3, Inf), at = 1:5)
text(c(1,2,3,4), .5,
     labels = c("n = 0", "n = 63", "n = 69", "n = 3", "n = 454"), cex = .5)
axis(side = 3, labels = round(rev(X_mantel_corlog_jaccard$mean.of.class),2), at = c(seq(1, 4, len = 7), 5))
text(c(seq(1, 4, len = 7), 5), 1,
     labels = c("n = 2", "n = 1", "n = 1", "n = 1", "n = 9", "n = 23", "n = 28", "n = 496"),
     cex = .5)
